...
plot_hist(x_thres , 20)

# generate data_list files
f = open('list.lst', 'w+')
for attr_ in attr_set:
    cur_list = T[attr_]
    for i, s in enumerate(cur_list):
        path = s.path.rstrip('.txt')
        dataset_prefix = 'dataset/ALOV/alov300++_frames/imagedata/'
        video_path = dataset_prefix + '/'.join(path.split('/')[3:])
        score_ = np.genfromtxt(s.path, dtype=[('index', int), ('score', float)], delimiter=',')
        for item in score_:
            f.write(video_path +'/ ' + str(item[0]) + ' ' + str(item[1]) + '\n')
f.close()
